...
const BehaviorAnalysis = () => {
  ...
  useEffect(() => {
    ...
    const fetchAffirmation = async () => {
      try {
        const prompt = `Give a postive affirmation to encourage in 80 words according to the behaviour analysis ${prediction}`;
    ...
    const analyzeBehavior = async () => {
      ...
      try {
        const prompt = "The user feels like " + feeling + " and has a diet of " + diet + " and exercises " + exercise + " and has a sleep of " + sleep + " hours and has a stress of " + stress + " out of 10. Then predict the behavior of the user. Give behaviour analysis in 80 words or less.";
        const predictionText = await getAffirmation({ prompt });
        setPrediction(predictionText);
      } catch (error) {
        setPrediction('Something went wrong, but keep your head up!');
      }
